...
页面展示选择器当前不会选中的账号，不读取或修改 `config.yaml` 的全局路由配置。分桶包括额度、认证与瞬时、待复查和重叠。看板每行直接提供“重检”和“恢复”：前者按当前限制执行额度复查或固定账号连接验证，后者经显式确认后按来源、模型和 revision 解除当前限制。账号详情仅展示账号与限制信息；账号巡检页仍保留完整的定向恢复面板，历史巡检结果与实时调度限制分别展示。
