# Account inspection batch and internal audit HTTP E2E
...
6. A suggested quota disable resolves to `quota_protection`, and a disconnected execute request still completes in the background with a queryable receipt.
7. A later suggested quota enable resolves to `quota_recovery` if the fixture produces quota usage data; the standard official xAI success response does not, so the receipt records this coverage gap.
8. An explicit disable resolves to `admin_disable`; a direct `/actions` operation keeps the same diagnostic fields in `before` and `after` while recording the execution effect.
9. A batch delete retains its diagnostic observation and operation evidence.
10. Killing Core during a blocked provider probe and restarting marks the running batch `interrupted`; the same operation cannot replay.
