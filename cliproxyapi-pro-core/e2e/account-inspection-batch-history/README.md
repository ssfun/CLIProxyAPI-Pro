...
The CI subset checks exact result filters plus legacy aliases, old `ResultRef` rejection, execute-time staleness, retry rebinding, quota protection, explicit quota override resolution, an old delete batch blocked by a newer manual disable, administrative disable, completion after an HTTP disconnect, retained delete evidence, bounded concurrent reinspection, recovery batch capacity, and evidence redaction. Quota recovery probing, restart recovery, and journal write-failure injection remain in the full run. In Validate, `result.json` and `server.log` are uploaded with Core validation diagnostics.
...
11. A serial recovery batch rejects more than 20 targets before execution.
